...
fn en_passant(random: &[u64]) -> EnPassantTable<Id> {
    let mut square = EnPassantTable::empty();
    for en_passant in EnPassant::ALL {
...
fn castle(random: &[u64]) -> PlayerTable<FileTable<Id>> {
    let mut players = PlayerTable::empty();
    let mut black = FileTable::empty();
    black[File::A] = id(random[771]);
    black[File::H] = id(random[770]);
    players[Player::Black] = black;

    let mut white = FileTable::empty();
    white[File::A] = id(random[769]);
    white[File::H] = id(random[768]);
    players[Player::White] = white;
...
fn write_rust(basis: Basis) -> io::Result<()> {
    let file = fs::File::create("src/id/polyglot.rs")?;
...
fn write_variant<W: Write>(writer: &mut Writer<W>, variant: VariantTable<Id>) -> io::Result<()> {
    writer.table_values(chess::position::VariantEnum::ALL, |writer, variant_enum| {
