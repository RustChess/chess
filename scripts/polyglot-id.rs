...
fn polyglot() -> Basis {
    ...
    Basis {
        board: board(&random),
        turn: PlayerTable::new([Id(0), id(random[780])]),
        castle: PlayerTable::new([
            SideTable::new([id(random[770]), id(random[771])]),
            SideTable::new([id(random[768]), id(random[769])]),
        ]),
        en_passant: en_passant(&random),
        variant: Variant { chess: Id(0), freestyle: Id(0) },
    }
}

fn board(random: &[u64]) -> SquareTable<PlayerTable<RoleTable<Id>>> {
    ...
    for square in Square::ALL {
        ...
        for player in Player::ALL {
            ...
            for role in Role::ALL {
                roles[role] = id(random[polyglot_piece(player, role) * 64 + square.index()]);
            }
            players[player] = roles;
        }
        squares[square] = players;
    }
    squares
}

fn en_passant(random: &[u64]) -> EnPassant {
    let mut square = en_passant::SquareTable::empty();
    for en_passant in en_passant::Square::ALL {
        square[en_passant] = id(random[772 + Square::from(en_passant).file().index()]);
    }

    EnPassant { none: Id(0), square }
}

fn polyglot_piece(player: Player, role: Role) -> usize {
    role.index() * 2 + player.index()
}
...
fn random64() -> Vec<u64> {
    RANDOM64
        ...
        .skip(1)
        .map(|text| {
            let (hex, _) = text.split_once(')').expect("Polyglot random value closes");
            u64::from_str_radix(hex, 16).expect("Polyglot random value is hex")
        })
        .collect()
}

fn write_rust(basis: Basis) -> io::Result<()> {
    let file = File::create("src/id/polyglot.rs")?;
    let mut writer = Writer::new(BufWriter::new(file));

    writer.line("// Generated by scripts/polyglot-id.rs.")?;
    writer.line(&format!("// Definitive reference: {SOURCE}"))?;
    writer.line("pub const POLYGLOT: Basis = Basis {")?;
    writer.indent();
    ...
    writer.field_struct("en_passant", "EnPassant", |writer| {
        write_en_passant(writer, basis.en_passant)
    })?;
    writer.field_struct("variant", "Variant", |writer| write_variant(writer, basis.variant))?;
    writer.dedent();
    writer.line("};")
}
...
fn write_castle<W: Write>(
    writer: &mut Writer<W>,
    castle: PlayerTable<SideTable<Id>>,
) -> io::Result<()> {
    writer.table_values(Player::ALL, |writer, player| {
        let sides = castle.get(player);
        writer.table("SideTable", Side::ALL, |writer, side| {
            write_id(writer, &format!("castle:{player}:{side}"), sides.get(side))
        })
    })
}

fn write_en_passant<W: Write>(writer: &mut Writer<W>, en_passant: EnPassant) -> io::Result<()> {
...
fn write_variant<W: Write>(writer: &mut Writer<W>, variant: Variant) -> io::Result<()> {
    writer.field_id("chess", "variant:chess", variant.chess)?;
    writer.field_id("freestyle", "variant:freestyle", variant.freestyle)
}
...
impl<W: Write> Writer<W> {
    ...
    fn field_id(&mut self, field: &str, key: &str, id: Id) -> io::Result<()> {
        self.line(&format!("{field}: Id({}), // {key}", hex(id.0)))
    }
